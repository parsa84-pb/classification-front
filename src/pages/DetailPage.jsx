...
const DetailPage = () => {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        const response = await api.get(`${baseURL}/api/frames/${frameId}/`);
  ...
  if (loading) {
    return (
      <div style={{ textAlign: "center", padding: "120px 0" }}>
        <Spin size="large" description="Compiling Deep Learning Reports..." />
      </div>
    );
  }
          ...
          >
            <img
              src={
                frameData.frame_image
                  ? frameData.frame_image.startsWith("http")
                    ? frameData.frame_image
                    : `${baseURL}${frameData.frame_image}`
                  : ""
              }
              alt="Analysis Core"
              style={{
            ...
            >
              <Descriptions.Item label="Source Type">
                <Tag color={isDirectUpload ? "blue" : "purple"}>
                  {isDirectUpload ? "Direct Photo Upload" : "Video Frame"}
                </Tag>
              </Descriptions.Item>
              {!isDirectUpload && (
                <>
                  <Descriptions.Item label="Timestamp">
                    <Space>
                      <ClockCircleOutlined />
                      {frameData.timestamp.toFixed(2)} seconds
                    </Space>
                  </Descriptions.Item>
                  <Descriptions.Item label="Original Video">
                    <a
                      href={frameData.video_source_url}
                      target="_blank"
                      rel="noreferrer"
                    >
                      <LinkOutlined /> Open Original Video
                    </a>
                  </Descriptions.Item>
                </>
              )}
        ...
        <Col xs={24} md={12}>
          ...
          >
            <Paragraph style={{ color: "#777", marginBottom: "24px" }}>
              Below are the Top-5 raw indices calculated by pooling softmax
              weights from ResNet50, EfficientNetB0, and MobileNetV3.
            </Paragraph>
              ...
              return (
                <div key={index} style={{ marginBottom: "20px" }}>
                  ...
                  >
                    <Text
                      strong={index === 0}
                      style={{ fontSize: index === 0 ? "16px" : "14px" }}
                    >
                      {index + 1}. {pred.class}
                    </Text>
                    <Text
                      type={index === 0 ? "success" : "secondary"}
                      strong={index === 0}
                    >
                      {pred.confidence}
                    </Text>
