...
import { EyeOutlined, CalendarOutlined, PictureOutlined } from '@ant-design/icons';
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '40px', borderBottom: '1px solid #f0f0f0', paddingBottom: '20px' }}>
        ...
        <Paragraph style={{ color: '#666' }}>
          Explore the repository of all video snapshots and uploaded images analyzed by our PyTorch Ensemble core.
        </Paragraph>
      </div>

      {/* Empty State if no data */}
      {frames.length === 0 ? (
        <Empty description="No analyzed items found yet. Go back and process some images or videos!" style={{ padding: '60px 0' }} />
      ) : (
        // Clean Grid Layout
        <Row gutter={[24, 24]}>
          {frames.map((item) => (
            <Col xs={24} sm={12} md={8} lg={6} key={item.id}>
              <Card
                hoverable
                style={{ borderRadius: '12px', overflow: 'hidden', boxShadow: '0 4px 10px rgba(0,0,0,0.03)' }}
                cover={
              ...
              >
                <div style={{ marginBottom: '16px', color: '#8c8c8c', fontSize: '13px' }}>
